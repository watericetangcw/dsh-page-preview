// dsh-page-preview — Client half v12 (function body returned as a Cordis Plugin)
// Feature 2 (floating window): movable, resizable bottom-right preview window.
// v12: content renders at a virtual desktop width (>=1024px) and scales to fit
// so small windows keep desktop layout proportions instead of squashing the
// page; closing morphs the window into the top-right capsule (shrink + move +
// round + fade, then capsule pops in). Icon toolbar + fullscreen retained.
  ...
  apply(ctx) {
    ...
    function InlinePreview(props) {
      ...
      React.useEffect(() => {
        ...
        const save = async () => {
          try {
            const res = await host.call('preview/snippet-save', { html })
    ...
    function PreviewWindow(props) {
      ...
      const refreshPane = () => {
        try { host.call('preview/refresh', { sessionId }).catch(() => {}) } catch (err) { /* ignore */ }
      }
    ...
    function PreviewOverlay(props) {
      ...
      React.useEffect(() => {
        ...
        const tick = async () => {
          ...
          let regKnown = true
          try {
            const res = await host.call('preview/state', { sessionId: sid })
            if (!alive) return
